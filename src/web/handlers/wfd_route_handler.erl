...
%% a module still can't be found, then the wfd_404 module is used if defined
...
try_load_module([_OneLeft], _ExtraTokens) -> undefined;
...
check_for_404(Module, PathInfo, Path) ->
    ...
    % is not, then try to load the wfd_404 page. If that
    % is not available, then default to the 'file_not_found_page' module.
    case code:ensure_loaded(Module) of
        {module, Module} -> {Module, PathInfo};
        _ -> 
            case code:ensure_loaded(wfd_404) of
                {module, wfd_404} -> {wfd_404, Path};
